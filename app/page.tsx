    ...
    const searchCustomers = async () => {
      ...
      try {
        ...
        if (matches.length === 0) {
          ...
          if (foundCustomer && !foundCustomer.id.startsWith("legacy_")) {
            // Recalculate points to ensure accuracy
            try {
              const correctPoints = await recalculateCustomerPoints(foundCustomer.id, foundCustomer.name)
              foundCustomer.rewardsPoints = correctPoints
            } catch (error) {
              console.warn("Could not recalculate points for customer:", error)
            }
          }
